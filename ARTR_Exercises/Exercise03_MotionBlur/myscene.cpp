...
void Manager::initialize()
{
	...
	auto lShaderGBufferVelo = ShaderManager::getShader({
		lPath + "shader/gbuffer.vert",
		lPath + "shader/gbuffer.frag"});

	auto lShaderTexture = ShaderManager::getShader({
		lPath + "shader/texture.vert",
		lPath + "shader/texture.frag"});
	...
	ECS.get<Transformation>(lEnt).scale(0.1f, 0.1f, 0.1f);
	ECS.get<Transformation>(lEnt).translate(0.0f, 0.0f, -10.f);
	...
	lEntOrbit.translate(0.0f, 5.0f, 0.0f);

	auto& lEntTrafo = addTransformationEntity();
	auto lIPRotation = addController<InterpolatedRotation>(entt::to_entity(ECS, lEntTrafo));
	lIPRotation->rotateStart(90., QVector3D(0.0, 0.0, 1.0));
	lIPRotation->rotateEnd(-90, QVector3D(0.0, 0.0, 1.0));
	...
	auto lShaderMotionBlur = ShaderManager::getShader({
		lPath + QString("shader/motionblur.vert"),
		lPath + QString("shader/motionblur.frag")
		});
