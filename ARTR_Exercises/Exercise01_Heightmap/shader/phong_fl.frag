//Dieser Shader besteht i.w. dem Phong_fl Shader, wobei material.diffuse und material.ambient durch
//die interpolierte Vertexfarbe ersetzt werden/wurden
...
{
    return clamp(material.emission.a + interpolatedVertexColor.a + interpolatedVertexColor.a + material.specular.a, 0.0, 1.0);
}

vec3 phongKdKaColor(vec3 lightViewDirection, vec3 viewNormal, vec3 lightAmbient, vec3 lightDiffuse, float lightAttenuation)
{
    vec3 diffuseColor = phongKd(lightViewDirection, viewNormal) * (lightDiffuse * interpolatedVertexColor.rgb);
    vec3 ambientColor = phongKa() * lightAmbient * interpolatedVertexColor.rgb;
