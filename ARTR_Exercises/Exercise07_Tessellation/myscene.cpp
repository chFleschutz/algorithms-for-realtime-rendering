...
void Manager::initialize()
{
	...
	auto lShaderBezier = ShaderManager::getShader({
		lPath + "shader/bezier.vert",
		lPath + "shader/basic.frag",
		QString(),
		lPath + "shader/bezier.tesc",
		lPath + "shader/bezier.tese" });

	auto lShaderTessCoverage = ShaderManager::getShader({
		lPath + "shader/tess.vert",
		lPath + "shader/basic.frag",
		QString(),
		lPath + "shader/dynamicLOD.tesc",
		lPath + "shader/screencoverage.tese" });

	auto shaderTessQuad = ShaderManager::getShader({
		lPath + "shader/tess.vert",
		lPath + "shader/basic.frag",
		QString(),
		lPath + "shader/dynamicLODQuad.tesc",
		lPath + "shader/quad.tese" });

	auto shaderTessQuadDisplace = ShaderManager::getShader({
		lPath + "shader/tess.vert",
		lPath + "shader/height.frag",
		lPath + "shader/basicgeo.geom",
		lPath + "shader/dynamicLODQuad.tesc",
		lPath + "shader/quadDisplace.tese" });

	auto shaderTessQuadLighting = ShaderManager::getShader({
		lPath + "shader/tess.vert",
		lPath + "shader/heightIllumination.frag",
		lPath + "shader/basicgeo.geom",
		lPath + "shader/dynamicLODQuad.tesc",
		lPath + "shader/quadDisplace.tese" });
	...
	auto lShaderTessCoverageWithHeight = ShaderManager::getShader({
		...
		lPath + "shader/heightIllumination.frag",
		lPath + "shader/basicgeo.geom",
		lPath + "shader/screencoverageHeight.tesc",
		lPath + "shader/screencoverageHeight.tese" });
	...
	ECS.get<Transformation>(lTessPlane).rotate(-90.0f, 1.0, 0.0f, 0.0f);
	...
	ECS.get<Transformation>(tessQuad).rotate(-90.0f, 1.0, 0.0f, 0.0f);
	...
	auto lTexture = std::make_shared<Texture>(lPath + "models/noiseTexture.png");
	lTexture->setNameString("heightMap");

	auto tessQuadDisplace = addRenderable<GeometryBase, TessQuad>(TessQuad(20.0f), shaderTessQuadDisplace);
	ECS.get<Transformation>(tessQuadDisplace).translate(-11.0, 0.0f, -50.0f);
	ECS.get<Transformation>(tessQuadDisplace).rotate(-90.0f, 1.0, 0.0f, 0.0f);
	ECS.get<Renderable>(tessQuadDisplace).addProperty(lTexture);
	ECS.get<Renderable>(tessQuadDisplace).addProperty(filled);

	auto tessQuadTerrain = addRenderable<GeometryBase, TessQuad>(TessQuad(20.0f), shaderTessQuadLighting);
	ECS.get<Transformation>(tessQuadTerrain).translate(11.0, 0.0f, -50.0f);
	ECS.get<Transformation>(tessQuadTerrain).rotate(-90.0f, 1.0, 0.0f, 0.0f);
	ECS.get<Renderable>(tessQuadTerrain).addProperty(lTexture);
	ECS.get<Renderable>(tessQuadTerrain).addProperty(filled);
	ECS.get<Renderable>(tessQuadTerrain).addProperty(std::make_shared<TerrainColor>());
